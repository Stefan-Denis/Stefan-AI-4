...
export default function audioToSubtitles(inputAudio, transcriptPath, outputSubtitlesPath) {
    return new Promise(async (resolve, reject) => {
        ...
        if (!inputAudio.endsWith('.wav')) {
            const ffmpegProcess = await new Promise((resolve, reject) => {
                ffmpeg()
                    .input(inputAudio)
        ...
        const timings = await parseTextGrid(outputFile);
